...
fun getProductOfStudentAges(students: List<Student>): Int {
    return students.asSequence().map { it.age }.reduce { it1, it2 -> it1 + it2}
}

// ignore F Grades
fun productOfStudentGrades(student: Student): Int {
    return student.grades.asSequence().filter { it.type != GradeType.F}.map{it.type.value}.reduce {it1, it2 -> it1 * it2}
}
